...
class Player():
    def __init__(self, x, y, theta, power, size, lives):
        self.x = x
        self.y = y
        self.theta = theta
        self.power = power
    ...
    def draw(self):
        ...
        rect = img.get_rect()
        rect.center = (self.x - math.cos(self.theta) * self.power, self.y - math.sin(self.theta) * self.power)
        win.blit(img, rect)
...
def show_go_screen():
    win.fill(BLUE)
    win.blit(background, background_rect)
    draw_text(win, 'BIRDS!', 200, WIDTH / 2, HEIGHT / 6, WHITE)
    ...
    draw_text(win, 'ESCAPE to pause', 30, WIDTH / 2, HEIGHT / 2 + 40 + 40, WHITE)
...
while running:
    if game_over:
        ...
        p = Player(p_ball.pos[0], p_ball.pos[1], 0, 10, 1.60 * p_ball.radius, 4)
        ball2 = Ball(300, 300, 1000000, 100, planet2)
        ball3 = Ball(600, 20000, 5000000000000, 150, planet3)
    ...
    if refresh:
        ...
        p_ball = Ball(300, 100, 1000000, 50, player_img)
        lives = p.lives
        p = Player(p_ball.pos[0], p_ball.pos[1], 0, 10, 1.60 * p_ball.radius, lives)
        ball2 = Ball(300, 300, 1000000, 100, planet2)
        ball3 = Ball(600, 20000, 5000000000000, 150, planet3)
        all_sprites.add(p_ball)
        obj_with_mass.add(p_ball)
